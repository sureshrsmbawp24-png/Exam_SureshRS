...
# 'Late_Delivery_Status' is excluded as it is assumed to be the target (Y).
FEATURE_NAMES = [
...
if st.button("Predict Late Delivery Status", type="primary"):
    if model is not None:
        # 1. Create DataFrame with the user's inputs
        input_df = pd.DataFrame([user_inputs])
        
        # 2. IMPORTANT: Reorder columns to ensure they match the model's training order
        ...
        try:
            ...
            # Display meaningful status message
            ...
            st.warning("Ensure the number and type of features match the model's training data.")
    else:
